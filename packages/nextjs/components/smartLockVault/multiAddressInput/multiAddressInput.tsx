...
interface IInputAddressProps {
  ...
  export const MultiAddressInput = ({ title, name, onConfirm, values,disabled=false, onDelete }: IInputAddressProps) => {
    ...
    return (
      <div className={`${styles.inputAddress} ${!!disabled ? styles.disabled :""}`}>
        ...
        <div className={`${styles.addressContainer} ${hasError ? styles.error : ""} mainAddressContainer ${!!disabled ? "disabled":""}`}>
          <input
          disabled={disabled}
            name={`${name}`}
            type="text"
